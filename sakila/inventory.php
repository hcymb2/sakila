...
if(isset($_POST["insert"]))
{
    ...
    $insertResult = mysqli_query($conn, "INSERT INTO inventory (film_id, store_id, last_update) VALUES('$film_id','$store_id', '$last_update');");


    if($insertResult){ 
		echo '<p class="msg">'."Record has been inserted successfuly.".'</p>';
		echo "<script>setTimeout(\"location.href = 'inventory.php';\",2000);</script>"; 
	}
	else{
...
if(isset($_GET["update"]))
{
	...
	$inventory_id = $_GET["update"];
		...
		$selResult = mysqli_query($conn, "SELECT * FROM inventory WHERE inventory_id = $inventory_id;");
...
if(isset($_POST["update"]))
{
	...
	$updateResult = mysqli_query($conn, "UPDATE inventory SET film_id='$film_id',store_id='$store_id', last_update='$last_update' WHERE inventory_id=$inventory_id;") or die($conn->error);
...
<html>
<head>
	<title>Sakila Database - Inventory Table</title>
...
<ul>
	...
	<li><a href="inventory.php">INVENTORY TABLE</a></li>
	...
	<form action="inventory.php" method="POST">
		<?php
			...
			<?php else: ?>
				...
				<div class="inputGrp">
					<label>film_id</label>
					<input type="number" name="film_id" value="" placeholder="film_id" > <br/> 
				</div>
				<div class="inputGrp">
					<label>store_id</label>
    ...
    <?php
		...
		$selectResult = mysqli_query($conn, "SELECT * FROM inventory;"); 
					...
					<table>
						<tr>
							...
							<?php 	if (!empty($_POST['selectField'])&&in_array("inventory_id",$_POST['selectField'])){ ?>
									...
									if (!empty($_POST['selectField'])&&in_array("film_id",$_POST['selectField'])){ ?>
									...
									<td><button type="submit" class="update_btn" name="update"><a href="inventory.php?update=<?php echo $row['inventory_id']; ?>">Update</a></button> </td>
									<td><button type="submit"  class="delete_btn" name="delete"><a href="inventory.php?delete=<?php echo $row['inventory_id']; ?>" onclick="return confirm('You are about to delete this record.');">Delete</a></button> </td>
