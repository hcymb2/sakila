...
if(isset($_POST["insert"]))
{
	...
	$sqlstmt = "INSERT INTO rental (rental_date, inventory_id, customer_id, return_date, staff_id, last_update) VALUES('$rental_date', '$inventory_id', '$customer_id', '$return_date', '$staff_id', '$last_update')";
	...
	if($insertResult){ //using javascript to setTimeout so we can display whether query was succesful or not message
		...
		echo "<script>setTimeout(\"location.href = 'rental.php';\",2000);</script>"; 
	}
	else{
...
if(isset($_GET["update"]))
{
	...
	$rental_id = $_GET["update"];
		...
		$selResult = mysqli_query($conn, "SELECT * FROM rental WHERE rental_id = $rental_id;");
...
if(isset($_POST["update"]))
{
	...
	$updateResult = mysqli_query($conn, "UPDATE rental SET rental_id='$rental_id', inventory_id='$inventory_id', customer_id='$customer_id', staff_id='$staff_id', return_date='$return_date', last_update='$last_update' WHERE rental_id=$rental_id;") or die($conn->error);
...
if (isset($_GET['delete'])) {
	...
	$deleteResult = mysqli_query($conn, "DELETE FROM rental WHERE rental_id= $rental_id");
...
<html>
<head>
	<title>Sakila Database - Rental Table</title>
...
<ul>
	...
	<li><a href="rental.php">RENTAL TABLE</a></li>
	...
	<form action="rental.php" method="POST">
        ...
        <input type="checkbox" name="selectField[]" value="rental_date" checked> <label>rental_date</label><br>
		<input type="checkbox" name="selectField[]" value="inventory_id" checked> <label>inventory_id</label><br>
        <input type="checkbox" name="selectField[]" value="customer_id" checked> <label>customer_id</label><br>
        <input type="checkbox" name="selectField[]" value="return_date" checked> <label>return_date</label><br>
		...
		<?php
			...
			?>
				...
				<div class="inputGrp">
					...
					<input type="date" name="rental_date" value="<?php echo $rental_date?>" > <br/> 
				</div>

				<div class="inputGrp">	
					<label>inventory_id</label>
					<input type="number" name="inventory_id" value="<?php echo $inventory_id?>" > <br/>
				</div>	
				<div class="inputGrp">
				...
				<div class="inputGrp">
					...
					<input type="date" name="return_date" value="<?php echo $return_date?>" > <br/> 
				</div>
                <div class="inputGrp">
			...
			<?php else: ?>
				...
				<div class="inputGrp">
					<label>rental_date</label>
					<input type="date" name="rental_date" value="rental_date"> <br/> 
                </div>

				<div class="inputGrp">
					<label>inventory_id</label>
					<input type="number" name="inventory_id" value="inventory_id" > <br/>
				</div>
				<div class="inputGrp">
					<label>customer_id</label>
				...
				<div class="inputGrp">
					<label>return_date</label>
					<input type="date" name="return_date" value="return_date"> <br/> 
                </div>
	...
	<?php
		...
		$selectResult = mysqli_query($conn, "SELECT * FROM rental;"); 
	...
	?>
					<table>
						<tr>
                            ...
                            <?php if(!empty($_POST['selectField'])&&in_array("rental_date",$_POST['selectField'])){ ?>
									<th>rental_date</th>
							<?php }
                                  else{ ?>  <th style="display:none;">rental_date</th>  <?php } ?>
                    
							<?php if(!empty($_POST['selectField'])&&in_array("inventory_id",$_POST['selectField'])){ ?>
									<th>inventory_id</th>
							<?php }
								  else{ ?>  <th style="display:none;">inventory_id</th>  <?php } ?>
                            ...
                            <?php if(!empty($_POST['selectField'])&&in_array("return_date",$_POST['selectField'])){ ?>
									<th>return_date</th>
							<?php }
                                  else{ ?>  <th style="display:none;">return_date</th>  <?php } ?>
							...
							<?php 	if (!empty($_POST['selectField'])&&in_array("rental_id",$_POST['selectField'])){ ?>
                                    ...
                                    if (!empty($_POST['selectField'])&&in_array("rental_date",$_POST['selectField'])){ ?>
										<td>  <?php echo $row['rental_date']; ?>  </td>  <?php }
									else{ ?>  <td style="display:none;"> <?php echo $row['rental_date']; ?>  </td>  <?php }
								
									if (!empty($_POST['selectField'])&&in_array("inventory_id",$_POST['selectField'])){ ?>
										<td>  <?php echo $row['inventory_id']; ?>  </td>  <?php }
									else{ ?>  <td style="display:none;"> <?php echo $row['inventory_id']; ?>  </td>  <?php }
                                    ...
                                    if (!empty($_POST['selectField'])&&in_array("return_date",$_POST['selectField'])){ ?>
										<td>  <?php echo $row['return_date']; ?>  </td>  <?php }
                                    else{ ?>  <td style="display:none;"> <?php echo $row['return_date']; ?>  </td>  <?php } 
									...
									<td><button type="submit" class="update_btn" name="update"><a href="rental.php?update=<?php echo $row['rental_id']; ?>">Update</a></button> </td>
									<td><button type="submit"  class="delete_btn" name="delete"><a href="rental.php?delete=<?php echo $row['rental_id']; ?>" onclick="return confirm('You are about to delete this record.');">Delete</a></button> </td>
